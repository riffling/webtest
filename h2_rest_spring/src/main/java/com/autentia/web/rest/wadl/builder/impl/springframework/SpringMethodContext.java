...
class SpringMethodContext extends MethodContext {
    ...
    protected String discoverPath() {
        ...
        if (path != null) {
            String controllName = handlerMethod.getBeanType().getName();
            String servletPrefix = RestControllerManager.getInstance().getServletPrefix(controllName);
            path = servletPrefix == null ? path : servletPrefix + path.replaceAll("//", "/");
        }
        return path;
    }
